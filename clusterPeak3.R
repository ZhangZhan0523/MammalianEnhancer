# for the chipseeker and basic clusterprofiler analysis of newly categorized enhancers and promoters after DV's Genome Research paper
...
option_list <- list(
    make_option(c("-s", "--species"),
        ...
        action = "store", help = "species name"
    ),
    make_option(c("-t", "--tissue"),
        type = "character", default = NULL,
        action = "store", help = "tissue name"
    ),
    make_option(c("-e", "--enhancer"),
        type = "character", default = NULL,
        action = "store", help = "enhancer peak file path"
    ),
    make_option(c("-p", "--promoter"),
        type = "character", default = NULL,
        action = "store", help = "promoter peak file path"
    ),
    make_option(c("-g", "--gff"),
        type = "character", default = NULL,
        action = "store", help = "genome annotation file, can be gff3 or gtf"
    ),
...
annotate <- lapply(peaks, annotatePeak,
    ...
    tssRegion = c(-1000, 1000), flankDistance = 1000000
)
...
sp_info <- read.csv("/media/Data/zhangz/chip/scripts/info/species.csv", header = TRUE, row.names = 11)
# genes <- lapply(peaks, function(i) seq2gene(i, c(-1000, 1000), 3000, txdb))
write.csv(enhancer_df, file = paste(species, tissue, "enhancer_flank.csv", sep = "_"), row.names = FALSE)
write.csv(promoter_df, file = paste(species, tissue, "promoter_flank.csv", sep = "_"), row.names = FALSE)
